...
export async function getArticles(): Promise<BlogPost[]> {
  await initDb();
  const pool = getPool();
  const { rows } = await pool.query<ArticleRow>('SELECT * FROM articles ORDER BY "createdAt" DESC');
...
export async function getArticleBySlug(slug: string): Promise<BlogPost | null> {
  await initDb();
  const pool = getPool();
  const { rows } = await pool.query<ArticleRow>('SELECT * FROM articles WHERE slug = $1', [slug]);
  if (!rows[0]) return null;
  return mapRowToArticle(rows[0]);
}
...
export async function updateArticle(
  ...
  if (updates.isCoralBadge !== undefined) {
    setClauses.push(`"isCoralBadge" = $${p++}`);
    values.push(updates.isCoralBadge);
  }
  if (updates.readingTime !== undefined) {
    setClauses.push(`"readingTime"  = $${p++}`);
  ...
  if (updates.tags !== undefined) {
    setClauses.push(`tags           = $${p++}`);
    values.push(JSON.stringify(updates.tags));
  }
  if (updates.author !== undefined) {
    setClauses.push(`author         = $${p++}`);
    values.push(JSON.stringify(updates.author));
  }
